import numpy as np
from lightfm.datasets.movielens import _build_interaction_matrix, _parse, _get_dimensions
...
class LightFMAlg:
    ...
    def __init__(self, loss_type, ds=None, labels=None, learning_rate=0.05, min_rating=4.0):
        ...
        print(self.train.shape[0], data["train"].shape[0])
    ...
    def generate_rec(self, model, user_id, num_known=5, num_rec=10):
        ...
        #indices = np.isin(self.labels[:,0], self.train.tocsr()[user_id].indices)
        #known_positives = self.labels[:,1][indices]
        test = self.train.tocsr()[user_id].indices
